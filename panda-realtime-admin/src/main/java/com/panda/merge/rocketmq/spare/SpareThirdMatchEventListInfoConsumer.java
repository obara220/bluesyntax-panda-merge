...
@Slf4j
@Component
public class SpareThirdMatchEventListInfoConsumer {
    ...
    public void initConsumer() throws MQClientException {
        ...
        // 创建消费者并设置配置
        consumer = new DefaultMQPushConsumer(CONSUME_REALTIME_GROUP + THIRD_MATCH_EVENT_LIST_INFO_API);
        ...
        // 订阅备用-MQ主题
        log.info("SpareThirdMatchEventListInfoConsumer 订阅主题,多条三方赛事事件={}", THIRD_MATCH_EVENT_LIST_INFO_API);
        consumer.subscribe(THIRD_MATCH_EVENT_LIST_INFO_API, "*");
        // 注册消息监听器
        ...
        for (MessageExt messageExt : msgs) {
            try {
                if (!realtimeSwitch && !realtimeEventSwitch) {
                    log.info("linkId={},topic={}--备用MQ向数据中心转发",messageExt.getKeys(),THIRD_MATCH_EVENT_LIST_INFO_API);
                    dataCenterProducer.send(messageExt,THIRD_MATCH_EVENT_LIST_INFO_API);
                    continue;
                }
                matchEventInfoProcessor.putMatchEventListInfo(messageExt,true);
            } catch (Exception ex) {
