...
public class SystemMarketPreApiImpl implements SystemMarketPreApi {
    ...
    public void saveSystemPreResultAndPush(String params) {
        ...
        log.info("::{}::收到(提前结算)系统级开关【SystemMarketPreApiImpl:saveSystemPreResultAndPush】:获取原redis参数:{}", linkId, mapRdis);
        int isEquals = 0;
        ...
        if (isEquals>0) {
            ...
            systemSrSwitchProducer.sendSystemSrSwitchMessage(linkId,SrOnOff);
            //比赛是否结束:matchOver
            ...
            if (AoOnOff == 0&&AoOnOff!=mapRdis.get("AO")) {
                ...
                log.info("::{}::查询需要关闭的AO赛事列表:{}",linkId,standardMatchInfoAOList);
            ...
            if (SrOnOff == 0&&SrOnOff!=mapRdis.get("SR")) {
                ...
                log.info("::{}::查询需要关闭的SR赛事列表:{}",linkId,standardMatchInfoSRList);
                standardMatchInfoList.addAll(standardMatchInfoSRList);
            }
            //刷新缓存
            redisService.hSetAll(SystemThirdMarketPreParams, maps);
            log.info("::{}::系统级提前结算开关切换,重新下发条数:{}",linkId,standardMatchInfoList.size());
            //循环并设置 提前结算标准盘口
            for (StandardMatchInfo standardMatchInfo : standardMatchInfoList) {
                ...
                String linkIds = linkId + "_" + standardMatchId + "_PRERESULT_SWIFT";
                ...
                for (String key : standardMatchMarketPreResultMessage) {
                    ...
                    log.info("::{}::刷新提前结算缓存:{}", linkId, marketMessage);
                ...
                if(marketPreResultMessageList.size()==0){
                    log.info("::{}::【SystemMarketPreApiImpl:saveSystemPreResultAndPush】:赛事对应提前结算盘口为空,不下发！",linkIds);
                    continue;
                }
                //下发盘口数据
                log.info("::{}::关闭提前结算,cashout=-2,下发提前结算数据到业务系统:{}",linkIds,marketPreResultMessageList);
                standardMatchPreResultProducer.sendStandardMatchPreResult(linkIds, standardMatchInfo, 1L, marketPreResultMessageList, marketPreResultMessageList.get(0).getMatchPreStatus(), System.currentTimeMillis());
            }
