    ...
    public void processMessageList(List<Request<MarketDBMessage>> requests) {
        ...
        if (datacenterOddsStatus == 1) {
            requests.forEach(req -> {
                ...
                // 转发消息到数据中心
                log.info("收到 ::{}:: Topic的消息：{}", THIRD_SPORT_MARKET_ODDS_UPDATE, req);
                // 【关键优化 1】构造包含 Tag 的目标地址
                // RocketMQ 发送时，如果需要带 Tag，必须写成 "Topic:Tag" 格式
                String toTopic = THIRD_SPORT_MARKET_ODDS_UPDATE + DATACENTER;
                ...
                log.info("LinkID: {} 消息已转发到数据中心，目标topic: {}, message:{}", key, toTopic, message);
            });
            return;
        }
