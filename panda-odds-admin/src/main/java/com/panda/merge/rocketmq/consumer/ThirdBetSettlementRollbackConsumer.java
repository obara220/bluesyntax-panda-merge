...
public class ThirdBetSettlementRollbackConsumer implements RocketMQListener<Request<ThirdBetSettlementRollbackDTO>> {
    ...
    public void onMessage(Request<ThirdBetSettlementRollbackDTO> request) {
        ...
        if (datacenterOddsStatus == 1) {
            // 转发消息到数据中心
            log.info("收到 ::{}:: Topic的消息：{}", THIRD_BET_SETTLEMENT_ROLLBACK_API, request.getData());
            String toTopic = THIRD_BET_SETTLEMENT_ROLLBACK_API + DATACENTER;
            String destination = !StringUtils.isEmpty(request.getTag()) ? toTopic + ":" + request.getTag() : toTopic;
            // 发送到 数据中心Topic
            MessageBuilder<Request<ThirdBetSettlementRollbackDTO>> builder = MessageBuilder.withPayload(request)
