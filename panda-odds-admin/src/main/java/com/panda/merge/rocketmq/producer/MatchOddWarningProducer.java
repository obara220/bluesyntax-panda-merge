...
public class MatchOddWarningProducer {
    ...
    public void sendMatchOddsWarningRisk(String linkId, Long standardMatchInfoId, Long marketCategoryId, boolean sign) {
        ...
        rocketMqTemplate.asyncSend("MATCH_ODDS_WARNING_RISK:" + standardMatchInfoId + ":" + marketCategoryId, builder.build(), new SendCallback() {
            @Override
            ...
            public void onException(Throwable throwable) {
                log.error("::{}::TOPIC={}，send fail; ", linkId, "MATCH_ODDS_WARNING_RISK", throwable);
            }
        });
        log.info("::{}::通知风控赔率告警解除,topic=MATCH_ODDS_WARNING_RISK,消息体:{}", linkId, JSON.toJSONString(matchOddsWarningMessage));
    }
}
