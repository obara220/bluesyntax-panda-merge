...
public class ThirdMarketResultConsumer implements RocketMQListener<Request<ThirdMatchResultDTO>> {
    ...
    @Override
    public void onMessage(Request<ThirdMatchResultDTO> request) {
        //数据中心赔率状态开关 1开 0关
        if (datacenterOddsStatus == 1) {
            // 转发消息到数据中心
            log.info("收到 ::{}:: Topic的消息：{}", THIRD_MARKET_RESULT_API, request.getData());
            String toTopic = THIRD_MARKET_RESULT_API + DATACENTER;
            String destination = !StringUtils.isEmpty(request.getTag()) ? toTopic + ":" + request.getTag() : toTopic;
            // 发送到 数据中心Topic
            MessageBuilder<Request<ThirdMatchResultDTO>> builder = MessageBuilder.withPayload(request)
