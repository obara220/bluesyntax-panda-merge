...
public class OutrightMarketSoldConsumer implements RocketMQListener<Request<OutrightMarketSoldMessage>> {
    ...
    @Override
    public void onMessage(Request<OutrightMarketSoldMessage> request) {
        //数据中心赔率状态开关 1开 0关
        if (datacenterOddsStatus == 1) {
            // 转发消息到数据中心
            log.info("收到 ::{}:: Topic的消息：{}", OUTRIGHT_MARKET_SOLD_MESSAGE, request.getData());
            String toTopic = OUTRIGHT_MARKET_SOLD_MESSAGE + DATACENTER;
            String destination = !StringUtils.isEmpty(request.getTag()) ? toTopic + ":" + request.getTag() : toTopic;
            // 发送到 数据中心Topic
            MessageBuilder<Request<OutrightMarketSoldMessage>> builder = MessageBuilder.withPayload(request)
        ...
        outrightMarketSoldProcessor.processOutrightMarketSold(request);
    }
}
