...
import static com.panda.merge.constant.ConstantSystem.STANDARD_SPORT_MARKET_ODDS_UPDATE;

@Slf4j
@Component
@DependsOn("oddsAdminApplication")
public class StandardSportMarketOddsUpdateMultipleConsumer extends
    ...
    private static final String CONSUMER_GROUP = "odds-group-" + STANDARD_SPORT_MARKET_ODDS_UPDATE;
    ...
    public MqConsumerConfig buildConfig() {
        ConsumerConfigDetail consumerConfigDetail = ConsumerConfigDetail.builder()
                                                                        ...
                                                                        .build();
        return new MqConsumerConfig(STANDARD_SPORT_MARKET_ODDS_UPDATE, CONSUMER_GROUP, new TypeReference<Request<MarketDBMessage>>() {
    ...
    public void processMessageList(List<Request<MarketDBMessage>> requests) {
        ...
        if (datacenterOddsStatus == 1) {
            requests.forEach(req -> {
                ...
                // 转发消息到数据中心
                log.info("收到 ::{}:: Topic的消息：{}", STANDARD_SPORT_MARKET_ODDS_UPDATE, req);
                // 【关键优化 1】构造包含 Tag 的目标地址
                // RocketMQ 发送时，如果需要带 Tag，必须写成 "Topic:Tag" 格式
                String toTopic = STANDARD_SPORT_MARKET_ODDS_UPDATE + DATACENTER;
        ...
        ListUtils.partition(requests, PARTITION_NUMS_PER_TIME).forEach(t -> standardSportMarketAndOddsBatchUpdateProxy.batchStandardOddsUpdate(t));
    }
}
