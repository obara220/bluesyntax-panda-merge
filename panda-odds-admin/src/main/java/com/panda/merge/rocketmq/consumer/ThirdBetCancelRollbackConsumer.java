...
public class ThirdBetCancelRollbackConsumer implements RocketMQListener<Request<ThirdBetCancelRollbackDTO>> {
    ...
    @Override
    public void onMessage(Request<ThirdBetCancelRollbackDTO> request) {
        //数据中心赔率状态开关 1开 0关
        if (datacenterOddsStatus == 1) {
            // 转发消息到数据中心
            log.info("收到 ::{}:: Topic的消息：{}", THIRD_BET_CANCEL_ROLLBACK_API, request.getData());
            String toTopic = THIRD_BET_CANCEL_ROLLBACK_API + DATACENTER;
            String destination = !StringUtils.isEmpty(request.getTag()) ? toTopic + ":" + request.getTag() : toTopic;
            // 发送到 数据中心Topic
            MessageBuilder<Request<ThirdBetCancelRollbackDTO>> builder = MessageBuilder.withPayload(request)
