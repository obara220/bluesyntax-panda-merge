...
public class UpdateOperateProxy {
    ...
    public StandardSportMarket updateStandardSportMarket(StandardSportMarket standardSportMarket, String linkId) {
        ...
        return standardSportMarketNewService.updateByPrimaryKeySelective(standardSportMarket);
    }
    ...
    public StandardSportMarketOdds updateStandardSportMarketOdds(StandardSportMarketOdds standardSportMarketOdds, String linkId) {
        ...
        log.info("::{}::执行 修改标准盘口赔率信息 数据库操作，MarketId={}, DataSourceCode={}, ThirdMarketSourceId={}, 时间:{}", linkId, standardSportMarketOdds.getMarketId(), standardSportMarketOdds.getDataSourceCode(), standardSportMarketOdds.getThirdOddsFieldSourceId(), TimeUtils.millsSecondsEast8ZoneGmt());
    ...
    public ThirdSportMarket updateThirdSportMarket(ThirdSportMarket thirdSportMarket, String linkId) {
        ...
        log.info("::{}::执行 修改第三方盘口信息 数据库操作，MatchId={}, DataSourceCode={}, ThirdMarketSourceId={}, 时间:{}", linkId, thirdSportMarket.getMatchId(), thirdSportMarket.getDataSourceCode(), thirdSportMarket.getThirdMarketSourceId(), TimeUtils.millsSecondsEast8ZoneGmt());
        return thirdSportMarketNewService.updateByPrimaryKeySelective(thirdSportMarket);
    }
    ...
    private boolean equalsThirdSportMarket(ThirdSportMarket old, ThirdSportMarket news, String linkId) {
        ...
        if ((old.getEventType() == null && news.getEventType() != null) || (old.getEventType() != null && news.getEventType() == null)) {
            log.info("::{}:: ThirdSportMarket新旧其中有一个为空，需要执行数据库调整操作 oldEventType={}, newsEventType={}, 时间:{}", linkId, old.getEventType(), news.getEventType(), TimeUtils.millsSecondsEast8ZoneGmt());
            return false;
        }
        if (old.getEventType() != null && news.getEventType() != null && !old.getEventType().equals(news.getEventType())) {
            log.info("::{}:: ThirdSportMarket新旧数据内容不一致，需要执行数据库调整操作 oldEventType={}, newsEventType={}, 时间:{}", linkId, old.getEventType(), news.getEventType(), TimeUtils.millsSecondsEast8ZoneGmt());
            return false;
        }
        return true;
    }
     ...
     * @return
     */
    public void updateThirdSportMarketOdds(List<ThirdSportMarketOdds> thirdSportMarketOddsList, List<ThirdMarketOddsDTO> thirdMarketOddsDTOS, ThirdSportMarket thirdSportMarket, String linkId) {
        List<ThirdSportMarketOdds> upThirdSportMarketOddsList = new ArrayList<>();
        if (dataSources.containsKey(thirdSportMarket.getDataSourceCode())) {
            for (ThirdSportMarketOdds thirdSportMarketOdds : thirdSportMarketOddsList) {
                ThirdSportMarketOdds old = thirdSportMarketOddsService.getItem(thirdSportMarket.getDataSourceCode(), thirdSportMarketOdds.getThirdOddsFieldSourceId(), thirdSportMarket.getId());
                if (old != null && equalsThirdSportMarketOdds(old, thirdSportMarketOdds, linkId)) {
                    log.info("::{}::当前修改第三方盘口赔率数据已为最新数据，跳过数据库修改操作，MatchId={}, DataSourceCode={}, ThirdMarketSourceId={}, 时间:{}", linkId, thirdSportMarket.getMatchId(), thirdSportMarket.getDataSourceCode(), thirdSportMarketOdds.getThirdOddsFieldSourceId(), TimeUtils.millsSecondsEast8ZoneGmt());
                    continue;
                }
                upThirdSportMarketOddsList.add(thirdSportMarketOdds);
            }
        } else {
            //其他数据源走原逻辑
            thirdSportMarketOddsService.upThirdOddsList(linkId, thirdSportMarket.getDataSourceCode(), thirdSportMarketOddsList, thirdMarketOddsDTOS);
            return;
        }
        log.info("::{}::执行 修改第三方盘口赔率信息 数据库操作，MatchId={}, DataSourceCode={}, ThirdMarketSourceId={},修改赔率条数：{}, 时间:{}", linkId, thirdSportMarket.getMatchId(), thirdSportMarket.getDataSourceCode(), thirdSportMarket.getThirdMarketSourceId(), upThirdSportMarketOddsList.size(), TimeUtils.millsSecondsEast8ZoneGmt());
        if (CollectionUtils.isEmpty(upThirdSportMarketOddsList)) {
            return;
        }
//        thirdSportMarketOddsService.upThirdOddsListByDataSourceCode(linkId, thirdSportMarket.getDataSourceCode(), upThirdSportMarketOddsList);
        //刷新LIST缓存
...
//        redisService.del(key);
        ...
        thirdSportMarketOddsList.forEach(odds -> {
            thirdSportMarketOddsService.updateByPrimaryKeySelective(odds.getDataSourceCode(), odds);
     ...
     * @return
     */
    private boolean equalsThirdSportMarketOdds(ThirdSportMarketOdds old, ThirdSportMarketOdds news, String linkId) {
        if ((old.getActive() == null && news.getActive() != null) || (old.getActive() != null && news.getActive() == null)) {
            log.info("::{}:: ThirdSportMarketOdds新旧其中有一个为空，需要执行数据库调整操作 oldActive={}, newsActive={}, 时间:{}", linkId, old.getActive(), news.getActive(), TimeUtils.millsSecondsEast8ZoneGmt());
            return false;
        }
        if (old.getActive() != null && news.getActive() != null && !old.getActive().equals(news.getActive())) {
            log.info("::{}:: ThirdSportMarketOdds新旧数据内容不一致，需要执行数据库调整操作 oldActive={}, newsActive={}, 时间:{}", linkId, old.getActive(), news.getActive(), TimeUtils.millsSecondsEast8ZoneGmt());
            return false;
        }
        if ((old.getOddsValue() == null && news.getOddsValue() != null) || (old.getOddsValue() != null && news.getOddsValue() == null)) {
            log.info("::{}:: ThirdSportMarketOdds新旧其中有一个为空，需要执行数据库调整操作 oldOddsValue={}, newsOddsValue={}, 时间:{}", linkId, old.getOddsValue(), news.getOddsValue(), TimeUtils.millsSecondsEast8ZoneGmt());
            return false;
        }
        if (old.getOddsValue() != null && news.getOddsValue() != null && !old.getOddsValue().equals(news.getOddsValue())) {
            log.info("::{}:: ThirdSportMarketOdds新旧数据内容不一致，需要执行数据库调整操作 oldOddsValue={}, newsOddsValue={}, 时间:{}", linkId, old.getOddsValue(), news.getOddsValue(), TimeUtils.millsSecondsEast8ZoneGmt());
            return false;
        }
        if ((old.getOriginalOddsValue() == null && news.getOriginalOddsValue() != null) || (old.getOriginalOddsValue() != null && news.getOriginalOddsValue() == null)) {
            log.info("::{}:: ThirdSportMarketOdds新旧其中有一个为空，需要执行数据库调整操作 oldOriginalOddsValue={}, newsOriginalOddsValue={}, 时间:{}", linkId, old.getOriginalOddsValue(), news.getOriginalOddsValue(), TimeUtils.millsSecondsEast8ZoneGmt());
            return false;
        }
        if (old.getOriginalOddsValue() != null && news.getOriginalOddsValue() != null && !old.getOriginalOddsValue().equals(news.getOriginalOddsValue())) {
            log.info("::{}:: ThirdSportMarketOdds新旧数据内容不一致，需要执行数据库调整操作 oldOriginalOddsValue={}, newsOriginalOddsValue={}, 时间:{}", linkId, old.getOriginalOddsValue(), news.getOriginalOddsValue(), TimeUtils.millsSecondsEast8ZoneGmt());
            return false;
        }
        if ((old.getOddsType() == null && news.getOddsType() != null) || (old.getOddsType() != null && news.getOddsType() == null)) {
            log.info("::{}:: ThirdSportMarketOdds新旧其中有一个为空，需要执行数据库调整操作 oldOddsType={}, newsOddsType={}, 时间:{}", linkId, old.getOddsType(), news.getOddsType(), TimeUtils.millsSecondsEast8ZoneGmt());
            return false;
        }
        if (old.getOddsType() != null && news.getOddsType() != null && !old.getOddsType().equals(news.getOddsType())) {
            log.info("::{}:: ThirdSportMarketOdds新旧数据内容不一致，需要执行数据库调整操作 oldOddsType={}, newsOddsType={}, 时间:{}", linkId, old.getOddsType(), news.getOddsType(), TimeUtils.millsSecondsEast8ZoneGmt());
            return false;
        }
        if ((old.getAddition1() == null && news.getAddition1() != null) || (old.getAddition1() != null && news.getAddition1() == null)) {
            log.info("::{}:: ThirdSportMarketOdds新旧其中有一个为空，需要执行数据库调整操作 oldAddition1={}, newsAddition1={}, 时间:{}", linkId, old.getAddition1(), news.getAddition1(), TimeUtils.millsSecondsEast8ZoneGmt());
            return false;
        }
        if (old.getAddition1() != null && news.getAddition1() != null && !old.getAddition1().equals(news.getAddition1())) {
            log.info("::{}:: ThirdSportMarketOdds新旧数据内容不一致，需要执行数据库调整操作 oldAddition1={}, newsAddition1={}, 时间:{}", linkId, old.getAddition1(), news.getAddition1(), TimeUtils.millsSecondsEast8ZoneGmt());
            return false;
        }
        if ((old.getNameCode() == null && news.getNameCode() != null) || (old.getNameCode() != null && news.getNameCode() == null)) {
            log.info("::{}:: ThirdSportMarketOdds新旧其中有一个为空，需要执行数据库调整操作 oldNameCode={}, newsNameCode={}, 时间:{}", linkId, old.getNameCode(), news.getNameCode(), TimeUtils.millsSecondsEast8ZoneGmt());
            return false;
        }
        if (old.getNameCode() != null && news.getNameCode() != null && !old.getNameCode().equals(news.getNameCode())) {
            log.info("::{}:: ThirdSportMarketOdds新旧数据内容不一致，需要执行数据库调整操作 oldNameCode={}, newsNameCode={}, 时间:{}", linkId, old.getNameCode(), news.getNameCode(), TimeUtils.millsSecondsEast8ZoneGmt());
            return false;
        }
        return true;
    }
