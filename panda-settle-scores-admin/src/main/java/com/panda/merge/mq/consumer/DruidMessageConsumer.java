...
@RocketMQMessageListener(
        ...
        consumerGroup = CONSUME_SETTLE_SCORE_GROUP + DATA_PUSH_DRUID_MESSAGE,
...
//            commonProducer.asyncSend(json, "datacenter-DATA_PUSH_DRUID_MESSAGE");
            return;
        }
        try {
            log.info("结算2.0服务数据库信息处理开始");
            ...
            log.info("::{}::topic={}结算2.0服务数据库信息处理结束,data={}", linkId, DATA_DRUID_MESSAGE_HANDLER, data);
        } catch (Exception e) {
            log.error("结算2.0服务监听数据库信息异常");
        }
    }
