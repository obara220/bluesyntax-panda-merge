...
//import com.panda.merge.check.IMatchSettleBatchCheckService;
...
//
//    @Autowired
//    IMatchDelaySettleInfoService matchDelaySettleInfoService;
//
//    @Autowired
//    MatchSettleGoalStatusRepository matchSettleGoalStatusRepository;
//
//    @Autowired
//    IMatchSettleEventService matchSettleEventService;
//
//    @Autowired
...
//    List<String> cornerDelaySettleNum = Arrays.asList("201","202","203","2011","2012","2014","2015","2013","2016");
//    List<String> bookingDelaySettleNum = Arrays.asList("308","304","309","301","302","305","306","303","307");
//
...
//    List<String> goal5SettleNum = Arrays.asList("1034", "1035", "1036", "1037", "1038", "1039", "1040","1041", "1042", "1044", "1045", "1046", "1047", "1048", "1049", "1050", "1051", "1052","1043","1053");
//    //重要核对校验阶段
//    @Override
...
//        example.createCriteria().andSettleNumIn(settleNums).andStandardMatchIdEqualTo(standardMatchId);
...
//            settleTemplateService.batchCancelGrayStatus(dataSourceCode, matchSettleScores);
...
//        checkInfoExample.createCriteria().andSettleScoreEventIdIn(scoreIds).andDataSourceCodeEqualTo(dataSourceCode);
...
//            }
//        }
//        matchSettleCheckInfoService.saveOrUpdateBatch(batchCheckInfos);
//        log.info("linkId::{} 完成当前数据商比分核对数据入库", linkedId);
//        if(!CollectionUtils.isEmpty(insertCheckInfos)) {
//            applicationContext.getBean(IMatchSettleBatchCheckService.class).validateDataScoreMismatch(insertCheckInfos, linkedId);
//        }
//
//        MatchSettleTemplate countDownTemplate = settleTemplateService.getTemplateByStandardMatchId(standardMatchId, SettleTemplateTypeEnum.COUNT_DOWEN.code);
//        log.info("linkId::{} TemplateJson {}", linkedId, countDownTemplate);
//        MatchSettleTemplate gayTemplate = settleTemplateService.getTemplateByStandardMatchId(standardMatchId,SettleTemplateTypeEnum.GRAY_AREA.code);
...
//            matchSettleCheckInfoService.saveOrUpdateBatch(updatedStatusCheckInfo);
...
//            batchSendCheckMessage(checkMessages, true, linkedId);
//        }
...
//                    || matchSettleCheckInfo.getDataSourceCode().equals("PD2") || matchSettleCheckInfo.getDataSourceCode().equals("TS")|| matchSettleCheckInfo.getDataSourceCode().equals("F01")) {
...
//        MatchSettleInfo matchSettleInfo = matchSettleInfoMapper.selectByPrimaryKey(matchSettleCheckInfo.getStandardMatchId());
...
//
//    @Override
//    public void batchCheckCommonMatchSettleScoreEvent(List<Pair<Object, MatchSettleCheckInfo>> checkMessages, boolean createCheck,
...
//        List<Long> scoreEventIds = checkMessages.stream().map(t->t.getRight().getSettleScoreEventId()).collect(Collectors.toList());
...
//            List<MatchSettleScore> settleScores = matchSettleScoreService.getByIds(scoreEventIds);
...
//            List<MatchSettleEvent> settleEvents = matchSettleEventService.getByIds(scoreEventIds);
...
//            if(alreadySettled) {
//                alreadySettledIds.add(t.getRight().getSettleScoreEventId());
//                return false;
//            }
//            return true;
//        }).collect(Collectors.toList());
//        matchDelaySettleInfoService.updateStatusByScoreIds(alreadySettledIds, SETTLED);
...
//        MatchSettleInfo matchSettleInfo = matchSettleInfoRepository.getMatchSettleInfo(matchSettleCheckInfoFirst.getStandardMatchId());
//        //5分钟编码 计算5分钟区间
//        List<String> fiveMinSettleNums = Arrays.asList("1034", "1035", "1036", "1037", "1038", "1039", "1040",
//                "1041", "1042", "1043", "1044", "1045", "1046", "1047", "1048", "1049", "1050", "1051", "1052", "1053");
//        //5分钟按钮不打开不做5分钟区间结算
...
//        Map<String, Integer> switchStatus = matchSettleDataSourceConfigService.getTournamentLevelStatuses(matchSettleCheckInfoFirst.getStandardMatchId(),matchSettleCheckInfoFirst.getDataSourceCode(),eventCodes);
//        checkMessages = checkMessages.stream().filter(t -> {
...
//
//        //比分类型灰色区间不走数据商自动结算
//        MatchSettleTemplate matchSettleTemplate = settleTemplateService.getTemplateByStandardMatchId(matchSettleCheckInfoFirst.getStandardMatchId(), SettleTemplateTypeEnum.DATA_SOURCE_WEIGHT.code);
//        if(matchSettleTemplate==null){
...
//        checkInfoExample.createCriteria().andSettleScoreEventIdIn(scoreEventId).andStandardMatchIdEqualTo(matchSettleCheckInfoFirst.getStandardMatchId());
//        List<MatchSettleCheckInfo> allMatchSettleCheckInfos = matchSettleCheckInfoMapper.selectByExample(checkInfoExample);
...
//            checkInfosForSettleMapWithDelay = batchMoveDelayCheckInfo(matchSettleCheckInfoFirst.getStandardMatchId(),checkInfosForSettleMap, linkedId);
//        }
...
//            Map<String, List<MatchSettleCheckInfo>> checkGroupMap = null;
//            Map<String, List<MatchSettleCheckInfo>> oldCheckGroupMap = null;
//            if ((!isScoreFlag)&& matchSettleInfo.getFiveMinSwitch() != null && matchSettleInfo.getFiveMinSwitch() == 1&&
//                    (matchSettleCheckInfo.getEventCode().equals("goal")|| matchSettleCheckInfo.getEventCode().equals("corner")||matchSettleCheckInfo.getEventCode().equals("yellow_card")||matchSettleCheckInfo.getEventCode().equals("red_card")||matchSettleCheckInfo.getEventCode().equals("fa_card"))) {
//                checkGroupMap = SettleCheckUtils.groupByFiveMinSettleCheck(list);
//                checkKey = SettleCheckUtils.countSettleEventFiveMinCompareKey(matchSettleCheckInfo);
//                oldCheckGroupMap = SettleCheckUtils.groupByFiveMinSettleCheck(oldList);
...
//                                    applicationContext.getBean(IMatchSettleBatchCheckService.class).pushStandardSettleScores(matchSettleCheckInfo.getStandardMatchId(),matchSettleCheckInfo.getEventCode());
//                                }
//                                if (matchSettleCheckInfo.getCheckType()==2){
...
//                                    applicationContext.getBean(IMatchSettleBatchCheckService.class).pushStandardSettleEvent(matchSettleCheckInfo.getStandardMatchId(),matchSettleCheckInfo.getEventCode());
//                                }
//                            }
//                        }
//
...
////                    //数据商阶段比分结算顺序拦截，如果之前的比分没有结算，则不能编辑当前的比分
//////                    if (matchSettleCheckInfo.getCheckDataType() == MatchSettleCheckConstant.CheckDataType.DATA_SOURCE && !this.isAllPeriodScoresBeforeSettled(matchSettleScore)) {
////                    if (matchSettleCheckInfo.getCheckDataType() == MatchSettleCheckConstant.CheckDataType.DATA_SOURCE && !this.isAllPeriodScoresBeforeSettled(matchSettleInfo, matchSettleScore, linkedId, settleNumMap)) {
...
////                            if(isScoreFlag){
////                                MatchSettleScore matchSettleScore = (MatchSettleScore)matchSettleScoreEvent;
////                                if(!this.checkSettleScoreAndAutoSettleNonEvent(matchSettleInfo, matchSettleScore,matchSettleCheckInfo)){
////                                    return;
////                                }
////                            }
//
...
//                        } else {
//                            log.error("linkedId::{} ethirdScoreEventId::{}::scoreEventId:{}:已满足条件但未触发结算,比分类型:{}", linkedId, matchSettleCheckInfo.getThirdSettleScoreEventId(),matchSettleCheckInfo.getSettleScoreEventId(), matchSettleCheckInfo.getCheckType());
//                        }
//                    } else {
//                        //统计理论结算时间 无数据商 取当前结算时间
//                        this.updateEventTime(eventTime, matchSettleScoreEvent);
...
//            }
//            if(!hasPassCheck){
//                nonPastCheckedMessages.add(Pair.of(matchSettleScoreEvent, matchSettleCheckInfo));
//            }
...
//                    continue;
//                }
//                if (!this.checkSettleScoreAndAutoSettleNonEvent(matchSettleInfo, matchSettleScore, matchSettleCheckInfo, linkedId, settleNumMap)) {
...
//            MatchSettleInfo matchSettleInfo = matchSettleInfoMapper.selectByPrimaryKey(matchSettleScore.getStandardMatchId());
//            matchSettleInfo.setCurrentEventTag(1);
//            matchSettleInfoMapper.updateByPrimaryKey(matchSettleInfo);
//        }
//    }
//
//    /**
...
//        MatchSettleCheckInfo matchSettleCheckInfoFirst = checkMessages.get(0).getRight();
//        MatchSettleDataSourceSwitchExample example = new MatchSettleDataSourceSwitchExample();
//        example.createCriteria().andSportIdEqualTo(matchSettleInfo.getSportId());
//        List<MatchSettleDataSourceSwitch> switches = matchSettleDataSourceSwitchMapper.selectByExample(example);
...
//            if (dataSourceSwitch.getCorner() == 1) {
...
//        if (matchSettleScoreEvent instanceof MatchSettleScore) {
//            MatchSettleScore settleScore = (MatchSettleScore) matchSettleScoreEvent;
//            List<MatchSettleCheckInfo> checkInfoList = allMatchSettleCheckInfoMap.get(settleScore.getId());
//            eventTime = searchEventTimeByScores(checkInfoList);
//        } else if (matchSettleScoreEvent instanceof MatchSettleEvent) {
//            MatchSettleEvent matchSettleEvent = (MatchSettleEvent) matchSettleScoreEvent;
//            List<MatchSettleCheckInfo> checkInfoList = allMatchSettleCheckInfoMap.get(matchSettleEvent.getId());
...
//
//                //如果当前操作人是操盘手，则无需更改次序
//                MatchSettleInfo matchSettleInfo = this.getMatchSettleInfoByStandardMatchId(matchSettleCheckInfo.getStandardMatchId());
...
//
//        MatchSettleInfo matchSettleInfo = this.getMatchSettleInfoByStandardMatchId(standardMatchId);
//        for (Pair<Object, MatchSettleCheckInfo> checkMessage : checkMessages) {
//            Object matchSettleScoreEvent  = checkMessage.getLeft();
//            MatchSettleCheckInfo matchSettleCheckInfo  = checkMessage.getRight();
//            if (matchSettleScoreEvent instanceof MatchSettleScore) {
...
//            matchSettleScoreService.saveOrUpdateBatch(updateScores);
//        }
//        if(!CollectionUtils.isEmpty(updateEvents)) {
//            matchSettleEventService.saveOrUpdateBatch(updateEvents);
//        }
//        if(!CollectionUtils.isEmpty(updateCheckInfos)) {
//            MatchSettleCheckInfoExample checkInfoExample = new MatchSettleCheckInfoExample();
//            checkInfoExample.createCriteria().andStandardMatchIdEqualTo(standardMatchId)
//                    .andUserNameIn(userNames).andSettleScoreEventIdIn(settleScoreEventIds);
//            List<MatchSettleCheckInfo> checkInfoList = matchSettleCheckInfoMapper.selectByExample(checkInfoExample);
...
//            matchSettleCheckInfoService.saveOrUpdateBatch(filteredUpdateCheckInfos);
//        }
...
//        }
//        MatchSettleInfo matchSettleInfo = this.getMatchSettleInfoByStandardMatchId(matchSettleScoreDtos.get(0).getStandardMatchId());
...
//        MatchSettleInfoExample example = new MatchSettleInfoExample();
//        example.createCriteria().andStandardMatchIdEqualTo(matchSettleCheckInfo.getStandardMatchId());
//        List<MatchSettleInfo> list = matchSettleInfoMapper.selectByExample(example);
//        if (list.size() == 0) {
//            log.error("standardMatchId:{},找不到赛事结算记录", matchSettleCheckInfo.getStandardMatchId());
//            return null;
//        }
//        MatchSettleInfo matchSettleInfo = list.get(0);
...
//            flag = false;
//        }
//        MatchSettleInfoExample exInfo = new MatchSettleInfoExample();
//        exInfo.createCriteria().andStandardMatchIdEqualTo(standardMatchId);
//        List<MatchSettleInfo> matchSettleInfos = matchSettleInfoMapper.selectByExample(exInfo);
...
//    }
//
//    private boolean isAllPeriodScoresBeforeSettled(MatchSettleInfo matchSettleInfo, MatchSettleScore matchSettleScore, String linkedId, Map<String, MatchSettleScore> settleNumMap ) {
//        boolean flag = true;
//        //查询赛事结算表 看是否关闭顺序结算控制 为开  (null or 0)
//        Long standardMatchId = matchSettleScore.getStandardMatchId();
//        if (standardMatchId == null || standardMatchId == 0L) {
...
//        //5分钟区间根据5分钟开关是否打开做校验
//        MatchSettleInfo matchSettleInfo = matchSettleInfoMapper.selectByPrimaryKey(matchSettleEvent.getStandardMatchId());
//        if (matchSettleInfo.getFiveMinSwitch() != null && matchSettleInfo.getFiveMinSwitch() == 1) {
...
//        String userName = "";
//        if (checkInfo.getCheckDataType().equals(2)) {
//            MatchSettleInfo matchSettleInfo = matchSettleInfoMapper.selectByPrimaryKey(checkInfo.getStandardMatchId());
//            if (matchSettleInfo.getSportId().equals(2L)){
...
//
//        MatchSettleInfo matchSettleInfo = matchSettleInfoRepository.getMatchSettleInfo(tempSettleScore.getStandardMatchId());
...
//            example.createCriteria().andSettleNumIn(settleNums).andStandardMatchIdEqualTo(tempSettleScore.getStandardMatchId()) ;
//            List<MatchSettleThirdScore> matchSettleThirdScores =matchSettleThirdScoreMapper.selectByExample(example);
...
//                }
//            }
//            List<MatchSettleOperateLog> operateLogs = iMatchSettleLogService.batchMatchSettleScoreAddLog(standardMatchInfo, matchSettleScore, userName,
//                    OperateLogTypeEnum.SCORE_SETTLE, OperateLogTypeEnum.CONFIRM_SCORE.getCode().toString(), "");
//            //2.MQ下发
//            if (matchSettleScore.getPeriodId()==100 && (matchSettleScore.getSettleNum().equals(MatchPeriodEnum.GOAL_10.getCode().toString()) ||
//                    matchSettleScore.getSettleNum().equals(MatchPeriodEnum.Corner_3.getCode().toString()) ||
...
//
//        //5.更新结算对象到结算表
//        matchSettleScoreService.saveOrUpdateBatch(batchUpdateSettleScores);
//        //更新延迟表
//        List<Long> scoreIds = batchUpdateSettleScores.stream().map(MatchSettleScore::getId).collect(Collectors.toList());
//        matchDelaySettleInfoService.updateStatusByScoreIds(scoreIds,3);
//
//        // 必须放在batchUpdateSettleScores后面
//        basketballMatchScoresSettleApi.verifyScoresIsSame(tempSettleScore.getStandardMatchId());
//        //结算时把回滚订单数清零
//        matchSettleService.batchSettleRollBackSetNullOrderCount(scoreIds);
//        matchSettleInfoRepository.updateMatchSettleInfoToRedis(matchSettleInfo,false);
//        matchSettleOperateLogService.saveOrUpdateBatch(batchUpdateOperateLogs);
...
//        MatchSettleInfoExample matchSettleInfoExample = new MatchSettleInfoExample();
//        matchSettleInfoExample.createCriteria().andStandardMatchIdEqualTo(matchSettleScore.getStandardMatchId());
//        List<MatchSettleInfo> matchSettleInfos = matchSettleInfoMapper.selectByExample(matchSettleInfoExample);
//        if (matchSettleInfos.size() != 0) {
//            MatchSettleInfo matchSettleInfo = matchSettleInfos.get(0);
//            matchSettleInfo.setModifyTime(System.currentTimeMillis());
...
//            }
//            //更新结算信息
//            matchSettleInfoMapper.updateByPrimaryKeySelective(matchSettleInfo);
//        } else {
...
//        criteria.andStandardMatchIdEqualTo(standardMatchId);
//        List<MatchSettleInfo> matchSettleInfos = matchSettleInfoMapper.selectByExample(matchSettleInfoExample);
//        if (matchSettleInfos.size() == 0 || matchSettleInfos.get(0) == null || matchSettleInfos.get(0).getSettleType() == 1) {
//            return false;
//        } else {
//            return true;
//        }
...
//        criteria.andStandardMatchIdEqualTo(standardMatchId);
//        List<MatchSettleInfo> matchSettleInfos = matchSettleInfoMapper.selectByExample(matchSettleInfoExample);
//        if (matchSettleInfos != null && matchSettleInfos.size() > 0) {
//            MatchSettleInfo matchSettleInfo = matchSettleInfos.get(0);
//            if (matchSettleInfo.getIsAutoSettleDataSource() == null || matchSettleInfo.getIsAutoSettleDataSource() == 0) {
//                return false;
//            } else {
...
//        try {
//            MatchSettleGoalStatus goalStatus = matchSettleGoalStatusMapper.selectByPrimaryKey(matchEventInfo.getThirdMatchId());
//            if (goalStatus == null) {
...
//                matchSettleGoalStatusMapper.insert(goalStatus);
...
//        MatchSettleGoalStatus goalStatus = matchSettleGoalStatusMapper.selectByPrimaryKey(thirdMatchId);
//        //BT 的先直接过去
...
//
//    @Override
//    public boolean isMatchGoalStatusConfirm(MatchSettleGoalStatus goalStatus, String eventCode) {
//        //BT 的先直接过去
//        if (eventCode.equals("goal")) {
//            if (goalStatus == null || goalStatus.getGoalStatus() == null || goalStatus.getGoalStatus() == MatchSettleCheckConstant.GoalStatus.CONFIRM) {
//                return true;
...
//            //删除事件标记赛事
//            MatchSettleInfo matchSettleInfo = matchSettleInfoMapper.selectByPrimaryKey(settleThirdEvent.getStandardMatchId());
...
//        }
//
//        MatchSettleInfo matchSettleInfo = this.getMatchSettleInfoByStandardMatchId(penaltyScoresVo.getStandardMatchId());
//        if (StringUtils.isNotEmpty(matchSettleInfo.getLimitUserArray())) {
//            JSONArray array = JSONArray.parseArray(matchSettleInfo.getLimitUserArray());
//            if (array.contains(operatorName)) {
...
//            return true;
//        }
//        StandardMatchInfo standardMatchInfo =standardMatchInfoMapper.selectByPrimaryKey(standardMatchId);
//        MatchSettleInfo matchSettleInfo = matchSettleInfos.get(0);
//        if (matchSettleInfo.getSettleOrderClosed() != null &&
//                matchSettleInfo.getSettleOrderClosed() != 0) {
//            return true;
...
//        }
//        MatchSettleScoreExample example = new MatchSettleScoreExample();
//        example.createCriteria().andSettleNumIn(settleNumList).andStandardMatchIdEqualTo(standardMatchInfo.getId()).andStatusIn(statusList);
//        List<MatchSettleScore> unsettledScores =matchSettleScoreMapper.selectByExample(example);
...
//        try {
//            MatchSettleInfo matchSettleInfo = matchSettleInfoMapper.selectByPrimaryKey(standardMatchId);
//            MatchSettleEventExample eventExample = new MatchSettleEventExample();
...
//        log.info("linkId::{} initDelaySettleScore start", linkedId);
...
//                        List<GrayAreaSettleDto> grayAreaSettleDtoList = SettleTemplateJsonUtils.tansferGrayAreaList(gayTemplate.getTemplateJson());
//                        Map<String, GrayAreaSettleDto> dataSourceGrayAreaOldMap = grayAreaSettleDtoList.stream().collect(Collectors.toMap(GrayAreaSettleDto::getDataSourceCode, obj -> obj, (key1, key2) -> key1));
//                        grayAreaSettleDto = dataSourceGrayAreaOldMap.getOrDefault(checkInfo.getDataSourceCode(), grayAreaSettleDto);
//                    }
...
//                example2.createCriteria().andStandardMatchIdEqualTo(batchCheckInfos.get(0).getStandardMatchId()).andCheckInfoIdIn(new ArrayList<>(validCheckInfoIds.keySet()));
...
//                    matchDelaySettleInfoService.saveOrUpdateBatch(batchDelay);
//                }
//            }
//        }catch(Exception e){
...
//        Map<Long, MatchDelaySettleInfo> delaySettleInfosMap = delaySettleInfos.stream().collect(Collectors.toMap(MatchDelaySettleInfo::getCheckInfoId, Function.identity()));
...
//    }
//
//    private Map<Long, List<MatchSettleCheckInfo>> batchMoveDelayCheckInfo(Long standardMatchId, Map<Long, List<MatchSettleCheckInfo>> checkInfoListByCheckInfoMap, String linkedId){
