...
@Slf4j
@Controller
public class MatchSettleCenterController {
    ...
    @Autowired
    MatchSettleScoreV2Repository matchSettleScoreRepository;
    @Autowired
    MatchSettleEventV2Repository matchSettleEventRepository;
    @Autowired
...
//    MatchGrayIntervalMapper matchGrayIntervalMapper;
    ...
    @Autowired
    MatchSettleAbnormalRepository matchSettleAbnormalRepository;
    @Autowired
    ...
    @Autowired
    MatchSettleDataSourceWeightConfigRepository matchSettleDataSourceWeightConfigRepository;
    @Autowired
    StandardSportMarketSellService standardSportMarketSellService;
    @Autowired
    MatchSettleScoreConverter matchSettleScoreConverter;
    @Autowired
    ...
    public Response MatchFreeze(MatchFreezeDto matchFreezeDto) {
        ...
        try {
            if (redisService.tryLock(key, key, 2, 5)) {
                MatchSettleInfoEntity oidMatchSettleInfo = matchSettleInfoRepository.getMatchSettleInfo(matchFreezeDto.getMatchId());
                ...
                List<MatchSettleEventEntity> matchSettleEvents = matchSettleEventRepository.selectByExample(example);
                if (matchSettleEvents.size() != 0) {
                    for (MatchSettleEventEntity event : matchSettleEvents) {
                        event.setSettleFreeze(matchFreezeDto.getFreezeSettleStatus());
                        event.setModifyTime(System.currentTimeMillis());

                    }
                    matchSettleEventRepository.updateBatchById(matchSettleEvents);
                }
                ...
                List<MatchSettleScoreEntity> matchSettleScores = matchSettleScoreRepository.selectByExample(scoreExample);
                if (matchSettleEvents.size() != 0) {
                    for (MatchSettleScoreEntity score : matchSettleScores) {
                        score.setSettleFreeze(matchFreezeDto.getFreezeSettleStatus());
                        score.setModifyTime(System.currentTimeMillis());
                        //matchSettleScoreRepository.updateById(score);
                    }
...
//        }
        ...
        List<MatchSettleScoreEntity> matchSettleScores = matchSettleScoreRepository.selectByExample(settleScoreExample);
        if (matchSettleScores.size() == 0) {
            return Response.failed("比分阶段冻结,比分信息为空!");
        }
        MatchSettleScoreEntity settleScore = matchSettleScores.get(0);
        ...
        MatchSettleScoreEntity matchSettleScore = matchSettleScores.get(0);
        ...
        //记录操作日志
        matchSettleLogService.scoresPeriodFreezeAddLog(standardMatchInfo, matchSettleScoreConverter.convertMatchSettleScoreEntityToScore(matchSettleScore), forw, scoresPeriodFreezeDto);


        matchSettleCenterProducer.operationLogScoresPeriodFreeze(standardMatchInfo, matchSettleScoreConverter.convertMatchSettleScoreEntityToScore(matchSettleScore), forw, scoresPeriodFreezeDto);
        //WS 推送
...
//                matchSettleScore.getEventCode())), "推送WS标准赛事结算比分" + matchSettleScore.getStandardMatchId());
    ...
    public Response ScoresPeriodOrderFreeze(ScoresPeriodOrderFreezeDto freezeDto) {
        ...
        MatchSettleEvent matchSettleEvent = matchSettleEventRepository.getById(eventsId);
    ...
    public Response settleSwitcher(MatchSettleSwitcherDto matchSettleSwitcherDto) {
        ...
        if (redisService.tryLock(redisKey, redisKey, 2, 5)) {
            ...
            //1.查询结算信息
            MatchSettleInfo matchSettleInfo = matchSettleInfoRepository.getOneFromDB(matchSettleSwitcherDto.getMatchId());
            log.info("matchSettleInfo get from db:"+matchSettleInfo);
            if (matchSettleInfo ==null && matchSettleSwitcherDto.getSettleType() == 1) {
                return Response.failed("1031958");
            }
            if (matchSettleInfo !=null && matchSettleInfo.getSettleType().equals(2) && matchSettleSwitcherDto.getSettleType().equals(2)) {
                return Response.failed("1031959");
            }

            log.info("::{}:: 查询结算信息: {} ", linkId, matchSettleInfo);
            //3.新增或更新结算信息, match_settle_info
            //记录修改前结算方式
            Integer settleType = 1;
            boolean createNewScore = false;
            if (matchSettleInfo != null) {
                settleType = matchSettleInfo.getSettleType();
            } else {
                createNewScore = true;
            }
            ...
            //7.记录操作日志
            matchSettleLogService.settleSwitcherAddLog(matchSettleInfo, matchSettleSwitcherDto, settleType);
    ...
    void settleAbnormalInit(Long standardMatchId, Long sportId) {
        ArrayList<MatchSettleAbnormalEntity> list = new ArrayList<>();
        ...
        matchSettleAbnormalRepository.saveBatch(list);
    }
     ...
     * @return
     */
    private ArrayList<MatchSettleAbnormalEntity> initMatchSettleAbnormalGoal(ArrayList<MatchSettleAbnormalEntity> list, Long standardMatchId, Long sportId) {
        //  ArrayList<MatchSettleAbnormal> list = new ArrayList<>();
        //上半场
        MatchSettleAbnormalEntity matchSettleAbnormal1 = FootBallMatchSettleScoreUtils.initMatchSettleAbnormal(standardMatchId, sportId);
...
*/
        ...
        //下半场
        MatchSettleAbnormalEntity matchSettleAbnormal3 = FootBallMatchSettleScoreUtils.initMatchSettleAbnormal(standardMatchId, sportId);
        ...
        //全场(常规赛)
        MatchSettleAbnormalEntity matchSettleAbnormal4 = FootBallMatchSettleScoreUtils.initMatchSettleAbnormal(standardMatchId, sportId);
        ...
        //加时赛上半场
        MatchSettleAbnormalEntity matchSettleAbnormal5 = FootBallMatchSettleScoreUtils.initMatchSettleAbnormal(standardMatchId, sportId);
...
/*        MatchSettleAbnormal matchSettleAbnormal5 = FootBallMatchSettleScoreUtils.initMatchSettleAbnormal(standardMatchId,sportId);
        ...
        //加时赛下半场
        MatchSettleAbnormalEntity matchSettleAbnormal6 = FootBallMatchSettleScoreUtils.initMatchSettleAbnormal(standardMatchId, sportId);
        ...
        //加时赛全场
        MatchSettleAbnormalEntity matchSettleAbnormal7 = FootBallMatchSettleScoreUtils.initMatchSettleAbnormal(standardMatchId, sportId);
        ...
        //点球总比分
        MatchSettleAbnormalEntity matchSettleAbnormal8 = FootBallMatchSettleScoreUtils.initMatchSettleAbnormal(standardMatchId, sportId);
        ...
        //点球前5轮比分
        MatchSettleAbnormalEntity matchSettleAbnormal9 = FootBallMatchSettleScoreUtils.initMatchSettleAbnormal(standardMatchId, sportId);
        ...
        //进球 0-15分钟
        MatchSettleAbnormalEntity matchSettleAbnormal10 = FootBallMatchSettleScoreUtils.initMatchSettleAbnormal(standardMatchId, sportId);
        ...
        //进球 15-30分钟
        MatchSettleAbnormalEntity matchSettleAbnormal11 = FootBallMatchSettleScoreUtils.initMatchSettleAbnormal(standardMatchId, sportId);
        ...
        //进球 30-45分钟
        MatchSettleAbnormalEntity matchSettleAbnormal12 = FootBallMatchSettleScoreUtils.initMatchSettleAbnormal(standardMatchId, sportId);
        ...
        //进球 45-60分钟
        MatchSettleAbnormalEntity matchSettleAbnormal13 = FootBallMatchSettleScoreUtils.initMatchSettleAbnormal(standardMatchId, sportId);
        ...
        //进球 60-75分钟
        MatchSettleAbnormalEntity matchSettleAbnormal14 = FootBallMatchSettleScoreUtils.initMatchSettleAbnormal(standardMatchId, sportId);
        ...
        //进球 75分钟-全场
        MatchSettleAbnormalEntity matchSettleAbnormal15 = FootBallMatchSettleScoreUtils.initMatchSettleAbnormal(standardMatchId, sportId);
     ...
     * @return
     */
    private ArrayList<MatchSettleAbnormalEntity> initMatchSettleAbnormalCorner(ArrayList<MatchSettleAbnormalEntity> list, Long standardMatchId, Long sportId) {
        // ArrayList<MatchSettleAbnormal> list = new ArrayList<>();
        //上半场
        MatchSettleAbnormalEntity matchSettleAbnormal1 = FootBallMatchSettleScoreUtils.initMatchSettleAbnormal(standardMatchId, sportId);
...
/*        MatchSettleAbnormal matchSettleAbnormal5 = FootBallMatchSettleScoreUtils.initMatchSettleAbnormal(standardMatchId,sportId);
     ...
     * @return
     */
    private ArrayList<MatchSettleAbnormalEntity> initMatchSettleAbnormalFaCard(ArrayList<MatchSettleAbnormalEntity> list, Long standardMatchId, Long sportId) {
        // ArrayList<MatchSettleAbnormal> list = new ArrayList<>();
        //上半场
        MatchSettleAbnormalEntity matchSettleAbnormal1 = FootBallMatchSettleScoreUtils.initMatchSettleAbnormal(standardMatchId, sportId);
...
/*        MatchSettleAbnormal matchSettleAbnormal5 = FootBallMatchSettleScoreUtils.initMatchSettleAbnormal(standardMatchId,sportId);
    ...
    public Response changeGlobalAutoSettleStatus(Boolean isEnableAutoSettle, String userName, String ipAddress) {
        ...
        MatchSettleInfoEntity record = new MatchSettleInfoEntity();
        ...
        matchSettleInfoRepository.updateByExampleSelective(record, settleInfoExample);
        ...
        matchSettleInfoRepository.updateByExampleSelective(record, settleInfoExample);
        matchSettleInfoHelper.deleteAllCacheBasedIdKeys();
        //记录日志
        ...
        String res = (status?"开":"关")+"-"+(isEnableAutoSettle?"开":"关");
        return Response.success("操作后 "+res);
    }
    ...
    public Response changeMatchAutoSettleStatus(String standardMatchId, Boolean isEnableAutoSettle, String userName, String ipAddress) {
        ...
        //查询未修改前的赛事数据源状态
        MatchSettleInfoEntity matchSettleInfo = matchSettleInfoRepository.getMatchSettleInfo(Long.valueOf(standardMatchId));
...
//        record.setGoalAutoSettleDataSource(record.getIsAutoSettleDataSource());
        ...
        if (isEnableAutoSettle) {
            redisService.del(CommonConstant.SETTLE_DATASOURCE_LOST_CONNECTION+standardMatchId);
            autoSettle = 1;
        }
        //需要恢复 篮球的 redis key
        if (isEnableAutoSettle) {
        ...
        wsPushService.pushGlobalAutoSettleStatus(autoSettleDataSourceDto);
        String res = (isAutoSettleDataSource == 1 ? "开":"关")+"-"+(isEnableAutoSettle?"开":"关");
        return Response.success("操作后 "+res);
    }
    ...
    public Response changeEventTypeAutoSettleStatus(String type, Long standardMatchId, Boolean isEnableAutoSettle, String userName, String ipAddress) {
        ...
        autoSettleDataSourceDto.setType(type);
        wsPushService.pushGlobalAutoSettleStatus(autoSettleDataSourceDto);
        String res = (oldAutoSettleDataSource == 1 ? "开":"关")+"-"+(isEnableAutoSettle?"开":"关");
        return Response.success("操作后 "+res);
    }
    ...
    public Response matchSettleAbnormal(List<SettleMatchScoreDto> list) {
        ...
        List<MatchSettleAbnormalEntity> matchSettleAbnormals = matchSettleAbnormalRepository.selectByExample(example);
        if (matchSettleAbnormals.size() == 0) {
            return Response.failed("103011");
        }
        for (MatchSettleAbnormalEntity matchSettleAbnormal : matchSettleAbnormals) {
            ...
            matchSettleAbnormalRepository.updateById(matchSettleAbnormal);
            ...
            //1.比分结算增加操作日志
            matchSettleLogService.matchSettleScoreAddLog(SettleScoreOid, matchSettleScore, list.get(0).getOperatorName(), OperateLogTypeEnum.SCORES_SETTLE_10039.getCode().toString(), before, list.get(0).getIpAddress());
        }
    ...
    public Response setSettleOrderClosed(MatchSettleOrderClosedDTO dto) {
        ...
        try {
            MatchSettleInfoEntity matchSettleInfo = matchSettleInfoRepository.getMatchSettleInfo(dto.getMatchId());
            if (matchSettleInfo != null) {
                ...
                matchSettleInfoRepository.updateMatchSettleInfoToRedis(matchSettleInfo, false);
                //1.比分结算增加操作日志
                matchSettleLogService.setSettleOrderClosedAddLog(info, matchSettleInfoConverter.convertMatchSettleInfoEntityToInfo(matchSettleInfo), dto, OperateLogTypeEnum.SCORES_SETTLE_10043);
        ...
        try {
            if (redisService.tryLock(key, key, 3, 5)) {
                ...
                if (matchSettleInfo != null) {
                    ...
                    matchSettleInfoRepository.updateMatchSettleInfoToRedis(matchSettleInfo, false);
                    //1.比分结算增加操作日志
                    matchSettleLogService.setFiveMinSwitchLog(info, matchSettleInfoConverter.convertMatchSettleInfoEntityToInfo(matchSettleInfo), dto, OperateLogTypeEnum.SCORES_SETTLE_10044);
...
//    }
    ...
    private void checkAndinitFiveMinScore(Long standardMatchId) {
        ...
        List<MatchSettleScoreEntity> list = matchSettleScoreRepository.selectByExample(example);
        if (list.size() == 0) {
    ...
    public Response getMatchSettleDataSources(MatchSettleDataSourceDto matchSettleDataSourceDto) {
        ...
        List<MatchSettleDataSourceConfig> matchSettleDataSourceConfigList = matchSettleDataSourceConfigRepository.selectByExample(matchSettleDataSourceConfigExample);
    ...
    public Response setLeagueMatchSettleDataSource(MatchSettleDataSourceDto matchSettleDataSourceDto) {
        ...
        try {
            ...
            if (matchSettleDataSourceDto.getTournamentLevel().equals(CategoryUtils.UN_LEVEL)) {
                ...
                List<MatchSettleDataSourceConfig> matchSettleDataSourceConfigUnLevelList = matchSettleDataSourceConfigRepository.selectByExample(matchSettleDataSourceConfigUnLevel);
                ...
                //将要更新的集合
                List<MatchSettleDataSourceConfig> updateWillDataSourceConfigEntityList = new ArrayList<>();
                ...
                for (MatchSettleDataSourceConfig settleDataSourceConfig : matchSettleDataSourceConfigList) {
                    ...
                    //matchSettleDataSourceConfigRepository.updateById(settleDataSourceConfig);
                    updateWillDataSourceConfigEntityList.add(settleDataSourceConfig);
                }

                if (!CollectionUtils.isEmpty(updateWillDataSourceConfigEntityList)){
                    matchSettleDataSourceConfigRepository.updateDataSourceConfigToRedis(updateWillDataSourceConfigEntityList.get(0), false);
                }

                matchSettleLogService.updateLeagueMatchSettleDataSourceLog(matchSettleDataSourceDto, oldStatus);
            } else {
                //更新单条联赛对应的数据源的开关
                MatchSettleDataSourceConfigExample matchSettleDataSourceConfigExample = new MatchSettleDataSourceConfigExample();
                matchSettleDataSourceConfigExample.createCriteria().andDataSourceCodeEqualTo(matchSettleDataSourceDto.getDataSourceCode())
                ...
                if (!matchSettleDataSourceConfigList.isEmpty()) {
                    ...
                    settleDataSourceConfig.setStatus(matchSettleDataSourceDto.getStatus());
                    settleDataSourceConfig.setModifyTime(System.currentTimeMillis());
                    matchSettleDataSourceConfigRepository.updateDataSourceConfigToRedis(settleDataSourceConfig,false);
                    matchSettleLogService.updateLeagueMatchSettleDataSourceLog(matchSettleDataSourceDto, oldStatus);
                } else {
                    Response.failed("没有联赛对应的数据源记录");
                }
    ...
    public Response setBasketInSettleTimeLimit(SettleTimeLimitDto dto) {
        ...
        if (dto.getLimitSwitchJson() != null && dto.getSportId() != null) {
            ...
            List<MatchSettleOperateLogEntity> willSaveOperateLogList = new ArrayList<>();
            oldConfigs.forEach(old -> {
                newConfigs.forEach(n -> {
                    if (old.getLevel() == n.getLevel()) {
                        if (old.getOnOff() != n.getOnOff()) { //记录开关日志
                            matchSettleLogService.editBasketBallSetUpConfigLog(old, n, dto, willSaveOperateLogList);
                        }
            ...
            if (!CollectionUtils.isEmpty(willSaveOperateLogList)) {
                matchSettleOperateLogV2Repository.saveOrUpdateBatch(willSaveOperateLogList);
            }
    ...
    public Response getMatchSettleDataSourcesWeight(MatchSettleDataSourceWeightDto matchSettleDataSourceWeightDto) {
        ...
        List<MatchSettleDataSourceWeightConfig> matchSettleDataSourceWeighConfigList = matchSettleDataSourceWeightConfigRepository.selectByExample(matchSettleDataSourceWeightConfigExample);
    ...
    public Response<Map<String, Object>> getRedisInfo(List<String> keys, String flag) {
        ...
        if ("DATASOURCE_CLOSE_LOST_CONNECTION".equals(flag)) {
            redisKeys = keys.stream().map(t->CommonConstant.SETTLE_DATASOURCE_LOST_CONNECTION+t).collect(Collectors.toList());
        }
        List<Object> values = redisService.mGet(redisKeys);
        int size = redisKeys.size();
        for (int i =0 ;i < size; i++) {
            if (values.get(i) != null) {
                objectMap.put(keys.get(i), values.get(i));
            }
        }
        return Response.success(objectMap);
    }

    public Response<List<DataSourceConnectionStatusDto>> scanAllMatchesConnectionStatus() {
        try {
            List<DataSourceConnectionStatusDto> result = dataSourceHeartbeatService.scanAllMatchesConnectionStatus();
            return Response.success(result);
        } catch (Exception e) {
            log.error("扫描所有赛事连接状态失败", e);
            return Response.failed("扫描失败: " + e.getMessage());
        }
    }
    ...
    public Response<DataSourceConnectionStatusDto> scanMatchConnectionStatus(Long standardMatchId) {
        try {
            DataSourceConnectionStatusDto result = dataSourceHeartbeatService.scanMatchConnectionStatus(standardMatchId);
            if (result == null) {
                return Response.failed("未找到该赛事的连接状态记录");
            }
            return Response.success(result);
        } catch (Exception e) {
