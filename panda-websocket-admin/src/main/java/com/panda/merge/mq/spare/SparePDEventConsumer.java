 ...
 *
 * @author warren
 * @since 2025/02/11 00:30:56
 */
@Slf4j
@Component
public class SparePDEventConsumer {
    private static final String TOPIC = "PD_FOOTBALL_EVENT";

    private static final String CONSUMER_GROUP = "scores-group-PDEventConsumer";
    ...
    public void initConsumer() throws MQClientException {
        log.info("SparePDEventConsumer:initConsumer");
        // 创建消费者并设置配置
        consumer = new DefaultMQPushConsumer(CONSUMER_GROUP);
        consumer.setInstanceName("SparePDEventConsumer");
        // Set name servers
        ...
        // 订阅备用-MQ主题
        log.info("SparePDEventConsumer 订阅主题,单条赛事事件={}", TOPIC);
        ...
        // 启动消费者
        consumer.start();
        log.info("SparePDEventConsumer started successfully");
    }
    ...
    public void shutdownConsumer() {
        if (consumer != null) {
            try {
                consumer.shutdown();
                log.info("SparePDEventConsumer,RocketMQ消费者已关闭");
            } catch (Exception e) {
                log.error("SparePDEventConsumer,关闭消费者时发生异常", e);
            }
        }
    }
    ...
    public void processMatchEvent(MessageExt ext) {
        ...
        try {
            ...
            if (StringUtils.isBlank(message)) {
                ...
                // 解析消息
                pdSubcribe.sendPdEvent(JSONObject.parseObject(message).getString("data"));
            }
        } catch (Exception e) {
