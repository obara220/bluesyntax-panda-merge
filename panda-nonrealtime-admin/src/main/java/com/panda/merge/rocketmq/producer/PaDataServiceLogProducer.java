    ...
    public void sendPaDataServiceLog(PaDataServiceLogDTO paDataServiceLogDTO) {
        try{
            if(pandaDataServiceLog){
                ...
                log.info("::{}::sendPaDataServiceLog 非实时服务统计PA数据服务日志消息结束,topic=PA_DATA_SERVICE_LOG", paDataServiceLogDTO.getLinkId());
            }
        }catch (Exception e){
            log.info("::"+paDataServiceLogDTO.getLinkId()+"::sendPaDataServiceLog 非实时服务统计PA数据服务日志消息异常,topic=PA_DATA_SERVICE_LOG,Exception:",e);
        }
    }
