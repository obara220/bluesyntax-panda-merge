...
//       }
    ...
    private void calcFootballEventTime(MatchEventInfo data, MatchScoresInfo matchScoresInfo) {
        ...
        //设置rediskey-加时赛
        String otHomeEventKey = "OT_POSSESSION:HOME:EVENT:MATCHID:"+data.getThirdMatchId();
        String otAwayEventKey = "OT_POSSESSION:AWAY:EVENT:MATCHID:"+data.getThirdMatchId();
        String otPublicEventKey = "OT_POSSESSION:PUB:EVENT:MATCHID:"+data.getThirdMatchId();
        //获取此刻上一次的事件
        MatchEventInfo lastEvent = getLastEventTime(data,homeEventKey,awayEventKey,publicEventKey);
        MatchEventInfo lastOtEvent = new MatchEventInfo();
        if(data.getMatchPeriodId().equals(41L) || data.getMatchPeriodId().equals(42L)){
            lastOtEvent = getLastEventTime(data,otHomeEventKey,otAwayEventKey,otPublicEventKey);
        }
        if(lastEvent==null){
        ...
        if(!Objects.equals(lastEvent.getMatchPeriodId(), data.getMatchPeriodId())){
            log.info("阶段变更，不计算控球率：{},currentPeriod：{}，lastEvent:{}",data.getLinkId(),data.getMatchPeriodId(),lastEvent.getMatchPeriodId());
        ...
        if(TeamTypeEnum.HOME.getCode().equals(lastEvent.getHomeAway())){
            buildPossessionData(wholeSores,possessionTime,TeamTypeEnum.HOME.getCode(), data.getLinkId(),periodSores);
        }else if(TeamTypeEnum.AWAY.getCode().equals(lastEvent.getHomeAway())){
            buildPossessionData(wholeSores,possessionTime,TeamTypeEnum.AWAY.getCode(), data.getLinkId(),periodSores);
        }else{
            buildPossessionData(wholeSores,possessionTime,"PUBLIC", data.getLinkId(),periodSores);
        }
...
//        matchScoreInfoRepository.updateScoresInfo(matchScoresInfo);
    ...
    private void buildPossessionData(FootballScores wholeSores,Integer possessionTime,String homeAway,String linkId,FootballScores periodSores){
        if(periodSores==null){
            return;
        }
        ...
        Integer otHomeAllTime = wholeSores.getPossessionTime().getHome();
        Integer otAwayAllTime = wholeSores.getPossessionTime().getAway();

        if(TeamTypeEnum.HOME.getCode().equals(homeAway)){
            homeAllTime += possessionTime;
            otHomeAllTime += possessionTime;
        }else if(TeamTypeEnum.AWAY.getCode().equals(homeAway)){
            awayAllTime += possessionTime;
            otAwayAllTime += possessionTime;
        }else{
            //上一个公共事件，不计算主客队控球时长
        }
        ...
        //添加控球时间，保存入库-阶段
        periodSores.getPossessionTime().setHome(otHomeAllTime);
        periodSores.getPossessionTime().setAway(otAwayAllTime);
        ...
        //控球率客A% =  事件持续时间A /  (事件持续时间H + 事件持续时间A )* 100 //控球率主H% = 事件持续时间H / (事件持续时间H + 事件持续时间A )* 100
        BigDecimal othomeTime = new BigDecimal(otHomeAllTime+"");
        BigDecimal otawayTime = new BigDecimal(otAwayAllTime+"");
        BigDecimal otHomeAndAway = othomeTime.add(otawayTime);
        if(otHomeAndAway.compareTo(BigDecimal.ZERO)==0){
            log.info("{},更新控球率信息：控球时长0",linkId);
            return;
        }
        ...
        BigDecimal otPossessionHome = othomeTime.divide(otHomeAndAway,2,4).multiply(onehundred);
        BigDecimal otPossessionAway = otawayTime.divide(otHomeAndAway,2,4).multiply(onehundred);
        ...
        periodSores.getBallPossessionPercentage().setHome(otPossessionHome.intValue());
        periodSores.getBallPossessionPercentage().setAway(otPossessionAway.intValue());
        log.info("{},更新控球率信息2：控球时长：{}-{}，控球率：{}-{},阶段控球率：{}-{}",linkId,homeAllTime,awayAllTime,
                wholeSores.getBallPossessionPercentage().getHome(),wholeSores.getBallPossessionPercentage().getAway(),
                periodSores.getBallPossessionPercentage().getHome(),periodSores.getBallPossessionPercentage().getAway());
    }
