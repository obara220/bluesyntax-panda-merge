...
@Slf4j
@Service
public class HockeyCalculationServiceImpl extends AbstractCalculationServiceImpl {
    ...
    private void updateScores(MatchScoresInfo matchScoresInfo, MatchEventInfo data) {
        if(!SportPeriodConstant.HockeyPeriod.contans(data.getMatchPeriodId())){
            return;
        }
        JSONObject periodFootballScores = JSONObject.parseObject(matchScoresInfo.getScoresJson());
        Map<Long, HockeyScores> allPeriodScores= JsonMapUtils.parseHockeyMap(periodFootballScores);
        ...
        HockeyScores periodScores= allPeriodScores.get(data.getMatchPeriodId());
        if(periodScores==null){
            periodScores =new HockeyScores();
...
//        matchScoreInfoRepository.updateScoresInfo(matchScoresInfo);
    ...
    public void cancelEvent(MatchScoresInfo matchScoresInfo, MatchEventInfo data , boolean isAgain,Boolean isReissue) throws Exception {
        ...
        Map<Long,HockeyScores> allPeriodScores= JsonMapUtils.parseHockeyMap(allscores);
        HockeyScores oldSores= allPeriodScores.get(data.getMatchPeriodId());
...
//        matchScoreInfoRepository.updateScoresInfo(matchScoresInfo);
    ...
    private void createScores(MatchScoresInfo matchScoresInfo, MatchEventInfo data) {
        Map<Long, HockeyScores> allPeriodScores= new HashMap<>();
        HockeyScores badmintonScores=new HockeyScores();
        allPeriodScores.put(WHOLE_MATCH,badmintonScores);
        allPeriodScores.put(data.getMatchPeriodId(),badmintonScores);

        //3.更新比分模板
        allPeriodScores.put(data.getMatchPeriodId(),((JSONObject) JSONObject.toJSON(badmintonScores)).toJavaObject(HockeyScores.class));
        matchScoresInfo.setScoresJson(JSONObject.toJSONString(allPeriodScores));
        matchScoresInfo.setModifyTime(System.currentTimeMillis());
//        matchScoreInfoRepository.updateScoresInfo(matchScoresInfo);
