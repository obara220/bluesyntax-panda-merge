...
@Component
@Slf4j
public class LiveDataScoresCsonumerThreadChangeListener implements NacosListener {
    ...
    public String nacosKeyToListen() {
        return LiveDataScoresNewConsumer.NACOS_MQ_CONSUMER_THREAD_NUMBER_KEY;
    }

    @Override
    public void onChange(NacosChangeEvent nacosChangeEvent) {
        DefaultMQPushConsumer consumer = liveDataScoresNewConsumer.getConsumer();
        consumer.updateCorePoolSize(Integer.parseInt(nacosChangeEvent.getValueAfter()));
        log.info("调整LiveDataScoresNewConsumer核心线程数成功,before:{},after:{}"
