...
import com.panda.merge.mq.consumer.SoldMessageStandardScoreConsumer;
...
public class SpareSoldMessageStandardScoreConsumer {
     ...
     */
    public static final String NACOS_MQ_CONSUMER_THREAD_NUMBER_KEY = "mq.sold-message-standard-score.consumer.thread";
    ...
    @Autowired
    private SoldMessageStandardScoreConsumer soldMessageStandardScoreConsumer;
    ...
    public void initConsumer() {
        ...
        if (StringUtils.isBlank(nameServers)) {
            log.warn("SpareSoldMessageStandardScoreConsumer: slaveNamesrvAddr配置为空，备用MQ消费者不启动");
            return;
        }
        try {
            log.info("SpareSoldMessageStandardScoreConsumer:initConsumer");
            // 创建消费者并设置配置
            consumer = new DefaultMQPushConsumer(CONSUMER_GROUP);
            consumer.setInstanceName("SpareSoldMessageStandardScoreConsumer");
            // Set name servers
            ...
            // 订阅备用-MQ主题
            log.info("SpareSoldMessageStandardScoreConsumer 订阅主题,开售消息标准比分={}", TOPIC);
            consumer.subscribe(TOPIC, "*");
            // 注册消息监听器
            consumer.registerMessageListener(this::processMessages);
            // 启动消费者
            consumer.start();
            log.info("SpareSoldMessageStandardScoreConsumer started successfully");
        } catch (Exception e) {
            log.error("SpareSoldMessageStandardScoreConsumer 启动失败，备用MQ消费者不启动: {}", e.getMessage(), e);
            // 不抛出异常，避免影响应用启动
        }
    }
    ...
    private void processMessage(MessageExt messageExt) {
        ...
        try {
            ...
            log.info("linkId=【{}】【备用-MQ,{}】开始处理开售消息标准比分数据", linkId, topic);
            ...
            SoldMessage data = jsonObject.getObject("data", SoldMessage.class);
            ...
            // 调用主消费者的onMessage方法
            soldMessageStandardScoreConsumer.onMessage(request);
            
        } catch (Exception e) {
            log.error("linkId=【{}】【备用-MQ,{}】开售消息标准比分数据处理异常: {}", linkId, topic, e.getMessage(), e);
    ...
    public void destroy() {
        if (consumer != null) {
            consumer.shutdown();
            log.info("SpareSoldMessageStandardScoreConsumer shutdown");
        }
    }
}
