...
import com.panda.merge.mq.consumer.MatchEndSettleEventConsumer;
...
/**
 * 备用同步结算事件到比分中心MQ消费（结算2.0手动操作下发比分）
 *

 */
@Slf4j
@Component
public class SpareMatchEndSettleEventConsumer {
    private static final String TOPIC = "MATCH_SETTLE_EVENT";
    private static final String CONSUMER_GROUP = "scores-group2-MATCH_SETTLE_EVENT-NEW2";
     ...
     */
    public static final String NACOS_MQ_CONSUMER_THREAD_NUMBER_KEY = "mq.settle-event.consumer.thread";
    ...
    @Autowired
    private MatchEndSettleEventConsumer matchEndSettleEventConsumer;
    ...
    public void initConsumer() {
        ...
        if (StringUtils.isBlank(nameServers)) {
            log.warn("SpareMatchEndSettleEventConsumer: slaveNamesrvAddr配置为空，备用MQ消费者不启动");
            return;
        }
        try {
            log.info("SpareMatchEndSettleEventConsumer:initConsumer");
            // 创建消费者并设置配置
            consumer = new DefaultMQPushConsumer(CONSUMER_GROUP);
            consumer.setInstanceName("SpareMatchEndSettleEventConsumer");
            // Set name servers
            ...
            // 订阅备用-MQ主题
            log.info("SpareMatchEndSettleEventConsumer 订阅主题,同步结算事件到比分中心={}", TOPIC);
            consumer.subscribe(TOPIC, "*");
            // 注册消息监听器
            consumer.registerMessageListener(this::processMessages);
            // 启动消费者
            consumer.start();
            log.info("SpareMatchEndSettleEventConsumer started successfully");
        } catch (Exception e) {
            log.error("SpareMatchEndSettleEventConsumer 启动失败，备用MQ消费者不启动: {}", e.getMessage(), e);
            // 不抛出异常，避免影响应用启动
        }
    }
    ...
    public void shutdownConsumer() {
        if (consumer != null) {
            try {
                consumer.shutdown();
                log.info("SpareMatchEndSettleEventConsumer,RocketMQ消费者已关闭");
            } catch (Exception e) {
                log.error("SpareMatchEndSettleEventConsumer,关闭消费者时发生异常", e);
            }
        }
    }
    ...
    public void processMessage(MessageExt ext) {
        ...
        try {
            ...
            log.info("linkId=【{}】【备用-MQ,{}】开始处理同步结算事件到比分中心数据", linkId, topic);
            String message = new String(ext.getBody(), StandardCharsets.UTF_8);
            if (StringUtils.isBlank(message)) {
                log.info("linkId=【{}】【备用-MQ,{}】接收到同步结算事件到比分中心数据为空！", linkId, topic);
                return;
            }
            // 解析消息
            JSONObject jsonObject = JSONObject.parseObject(message);
            MatchSettleEvent data = jsonObject.getObject("data", MatchSettleEvent.class);
            ...
            // 调用主消费者的处理方法
            matchEndSettleEventConsumer.onMessage(request);
        } catch (Exception e) {
            log.error("linkId=【{}】【备用-MQ,{}】同步结算事件到比分中心数据处理异常,Exception:", linkId, topic, e);
        } finally {
            log.info("linkId=【{}】【备用-MQ,{}】同步结算事件到比分中心数据处理结束", linkId, topic);
        }
    }
}
