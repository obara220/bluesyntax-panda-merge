...
public class SpareStandMatchScoresUpdateConsumer {
    ...
    public void initConsumer() {
        ...
        if (StringUtils.isBlank(nameServers)) {
            log.warn("SpareStandMatchScoresUpdateConsumer: slaveNamesrvAddr配置为空，备用MQ消费者不启动");
            return;
        }
        try {
            log.info("SpareStandMatchScoresUpdateConsumer:initConsumer");
            // 创建消费者并设置配置
            consumer = new DefaultMQPushConsumer(CONSUMER_GROUP);
            consumer.setInstanceName("SpareStandMatchScoresUpdateConsumer");
            // Set name servers
            ...
            // 订阅备用-MQ主题
            log.info("SpareStandMatchScoresUpdateConsumer 订阅主题,标准比分更新={}", TOPIC);
            consumer.subscribe(TOPIC, "*");
            // 注册消息监听器
            consumer.registerMessageListener(this::processMessages);
            // 启动消费者
            consumer.start();
            log.info("SpareStandMatchScoresUpdateConsumer started successfully");
        } catch (Exception e) {
            log.error("SpareStandMatchScoresUpdateConsumer 启动失败，备用MQ消费者不启动: {}", e.getMessage(), e);
            // 不抛出异常，避免影响应用启动
        }
    }
    ...
    public void shutdownConsumer() {
        if (consumer != null) {
            try {
                consumer.shutdown();
                log.info("SpareStandMatchScoresUpdateConsumer,RocketMQ消费者已关闭");
            } catch (Exception e) {
                log.error("SpareStandMatchScoresUpdateConsumer,关闭消费者时发生异常", e);
            }
        }
    }
    ...
    public void processMessageList(MessageExt ext) {
        ...
        try {
            ...
            log.info("linkId=【{}】【备用-MQ,{}】开始处理标准比分更新数据", linkId, topic);
            String message = new String(ext.getBody(), StandardCharsets.UTF_8);
            if (StringUtils.isBlank(message)) {
                log.info("linkId=【{}】【备用-MQ,{}】接收到标准比分更新数据为空！", linkId, topic);
                return;
            }
            // 解析消息
            JSONObject jsonObject = JSONObject.parseObject(message);
            // 解析为批量消息列表
            List<StandardMatchScores> dataList = JSONObject.parseObject(jsonObject.getString("data"), new TypeReference<List<StandardMatchScores>>() {
            });
            ...
            // 转换为Request列表
            List<Request<StandardMatchScores>> requestList = new ArrayList<>();
            for (StandardMatchScores data : dataList) {
                Request<StandardMatchScores> request = new Request<>();
                request.setSpareMq(true);
                request.setLinkId(linkId);
                request.setTag(data.getMatchId() != null ? data.getMatchId().toString() : linkId);
            ...
            // 调用主消费者的批量处理方法
            standMatchScoresUpdateConsumer.processMessageList(requestList);
        } catch (Exception e) {
            log.error("linkId=【{}】【备用-MQ,{}】标准比分更新数据处理异常,Exception:", linkId, topic, e);
        } finally {
            log.info("linkId=【{}】【备用-MQ,{}】标准比分更新数据处理结束", linkId, topic);
        }
    }
}
