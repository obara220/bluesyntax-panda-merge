...
import com.panda.merge.mq.consumer.MatchScoreSourceTypeConsumer;
...
public class SpareMatchScoreSourceTypeConsumer {
     ...
     */
    public static final String NACOS_MQ_CONSUMER_THREAD_NUMBER_KEY = "mq.source-type.consumer.thread";
    ...
    @Autowired
    private MatchScoreSourceTypeConsumer matchScoreSourceTypeConsumer;
    ...
    public void initConsumer() {
        ...
        if (StringUtils.isBlank(nameServers)) {
            log.warn("SpareMatchScoreSourceTypeConsumer: slaveNamesrvAddr配置为空，备用MQ消费者不启动");
            return;
        }
        try {
            log.info("SpareMatchScoreSourceTypeConsumer:initConsumer");
            // 创建消费者并设置配置
            consumer = new DefaultMQPushConsumer(CONSUMER_GROUP);
            consumer.setInstanceName("SpareMatchScoreSourceTypeConsumer");
            // Set name servers
            ...
            // 订阅备用-MQ主题
            log.info("SpareMatchScoreSourceTypeConsumer 订阅主题,比分修改SourceType={}", TOPIC);
            consumer.subscribe(TOPIC, "*");
            // 注册消息监听器
            consumer.registerMessageListener(this::processMessages);
            // 启动消费者
            consumer.start();
            log.info("SpareMatchScoreSourceTypeConsumer started successfully");
        } catch (Exception e) {
            log.error("SpareMatchScoreSourceTypeConsumer 启动失败，备用MQ消费者不启动: {}", e.getMessage(), e);
            // 不抛出异常，避免影响应用启动
        }
    }
    ...
    public void shutdownConsumer() {
        if (consumer != null) {
            try {
                consumer.shutdown();
                log.info("SpareMatchScoreSourceTypeConsumer,RocketMQ消费者已关闭");
            } catch (Exception e) {
                log.error("SpareMatchScoreSourceTypeConsumer,关闭消费者时发生异常", e);
            }
        }
    }
    ...
    public void processMessage(MessageExt ext) {
        ...
        try {
            ...
            log.info("linkId=【{}】【备用-MQ,{}】开始处理比分修改SourceType数据", linkId, topic);
            String message = new String(ext.getBody(), StandardCharsets.UTF_8);
            if (StringUtils.isBlank(message)) {
                log.info("linkId=【{}】【备用-MQ,{}】接收到比分修改SourceType数据为空！", linkId, topic);
                return;
            }
            // 解析消息
            JSONObject jsonObject = JSONObject.parseObject(message);
            MatchScoresSourceType data = jsonObject.getObject("data", MatchScoresSourceType.class);
            ...
            // 调用数据处理逻辑
            Request<MatchScoresSourceType> request = new Request<>();
            request.setSpareMq(true);
            request.setLinkId(linkId != null ? linkId : jsonObject.getString("linkId"));
            request.setTag(data.getThirdMatchId() != null ? data.getThirdMatchId().toString() : linkId);
            request.setDataType(topic);
            request.setData(data);
            // 调用主消费者的处理方法
            matchScoreSourceTypeConsumer.onMessage(request);
        } catch (Exception e) {
            log.error("linkId=【{}】【备用-MQ,{}】比分修改SourceType数据处理异常,Exception:", linkId, topic, e);
        } finally {
            log.info("linkId=【{}】【备用-MQ,{}】比分修改SourceType数据处理结束", linkId, topic);
        }
    }
}
