...
import com.panda.merge.mq.consumer.MatchSettleNewEventConsumer;
...
public class SpareMatchSettleNewEventConsumer {
    ...
    @Autowired
    private MatchSettleNewEventConsumer matchSettleNewEventConsumer;
    ...
    public void initConsumer() {
        ...
        if (StringUtils.isBlank(nameServers)) {
            log.warn("SpareMatchSettleNewEventConsumer: slaveNamesrvAddr配置为空，备用MQ消费者不启动");
            return;
        }
        try {
            log.info("SpareMatchSettleNewEventConsumer:initConsumer");
            // 创建消费者并设置配置
            consumer = new DefaultMQPushConsumer(CONSUMER_GROUP);
            consumer.setInstanceName("SpareMatchSettleNewEventConsumer");
            // Set name servers
            ...
            // 订阅备用-MQ主题
            log.info("SpareMatchSettleNewEventConsumer 订阅主题,结算2.0事件下发比分={}", TOPIC);
            consumer.subscribe(TOPIC, "*");
            // 注册消息监听器
            consumer.registerMessageListener(this::processMessages);
            // 启动消费者
            consumer.start();
            log.info("SpareMatchSettleNewEventConsumer started successfully");
        } catch (Exception e) {
            log.error("SpareMatchSettleNewEventConsumer 启动失败，备用MQ消费者不启动: {}", e.getMessage(), e);
            // 不抛出异常，避免影响应用启动
        }
    }
    ...
    public void shutdownConsumer() {
        if (consumer != null) {
            try {
                consumer.shutdown();
                log.info("SpareMatchSettleNewEventConsumer,RocketMQ消费者已关闭");
            } catch (Exception e) {
                log.error("SpareMatchSettleNewEventConsumer,关闭消费者时发生异常", e);
            }
        }
    }
    ...
    public void processMessage(MessageExt ext) {
        ...
        try {
            ...
            // 调用主消费者的处理方法
            matchSettleNewEventConsumer.onMessage(request);
        } catch (Exception e) {
