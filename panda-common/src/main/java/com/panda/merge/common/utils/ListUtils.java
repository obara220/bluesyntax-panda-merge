...
public class ListUtils {
     ...
     * @param isAsc
     *            true升序，false降序
     *  @param paOddsValue  低赔
     * @param oddsMetric  赔率差绝对值
            ...
            public int compare(E a, E b) {
                ...
                try {
                    for (int i = 0; i < sortnameArr.length; i++) {
                        /**
                         * 2、盘口赔率差绝对值相同的情况：
                         * 1）两边的值相等，当两边的赔率值相等时即赔率差为0，取欧赔更大的值，比如 1.9 1.9 为主盘 1.8 1.8为副盘；
                         * 2）两边的值不相等低赔最小的盘口为主盘，排在上面，低赔大的盘口为副盘，排在下面。这里说的低赔，指盘口各投注项中的最小赔率；
                         **/
                        if(sortnameArr[i].equalsIgnoreCase("paOddsValue")){
                            Object value1 = ListUtils.forceGetFieldValue(a, "oddsMetric");
                            Object value2 = ListUtils.forceGetFieldValue(b, "oddsMetric");
                            if(null != value1 && null != value2 && (int)value1 == 0 && (int)value2 == 0){
                                //降序排列
    ...
    private static <E> int compareObject(final String sortname, final boolean isAsc, E a, E b) throws Exception {
        ...
        if(value1 instanceof Long && value2 instanceof Long){
            if (isAsc) {
                ret = Long.compare(Long.parseLong(str1), Long.parseLong(str2));
            } else {
                ret = Long.compare(Long.parseLong(str2), Long.parseLong(str1));
            }
        }else{
