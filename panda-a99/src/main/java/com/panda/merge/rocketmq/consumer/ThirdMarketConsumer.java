...
@DependsOn("a99AdminApplication")
public class ThirdMarketConsumer extends AbstractMultipleMessageMQConsumer<Request<ThirdMatchMarketDTO>> {

    private static final String CONSUMER_GROUP = CONSUMER_PANDA_A99_GROUP + THIRD_TX_MATCH_MARKET_API;
    ...
    @Override
    protected MqConsumerConfig buildConfig() {
    ...
    @Override
    protected void processMessageList(List<Request<ThirdMatchMarketDTO>> list) {
        ListUtils.partition(list, PARTITION_NUMS_PER_TIME).forEach(t -> thirdMatchMarketProcessor.execute(t));
    }
}
