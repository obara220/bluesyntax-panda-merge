...
@Validated
@Component
@Async("ProcessA99ThirdMarketThreadPool")
public class A99ThirdAllBatchMarketProcessor extends BaseProcessor {
    @Autowired
    ...
    @Autowired
    private A99CalculationMarketProcessor myCalculationMarketProcessor;
    @Autowired
...
//                log.info("::{}::百家赔:接收数据源赔率开始,玩法id:{} ,异步处理数据开始", linkId, thirdCategorySourceId);
                ...
                for (ThirdMarketDTO thirdMarketDTO : value) {
                    ...
                    thirdSportMarketMessage.setDataSourceCode(internalDataSourceCode);
                    ...
                    if (!CollectionUtils.isEmpty(thirdMarketDTO.getMarketOddsList())) {
                        for (ThirdMarketOddsDTO thirdMarketOddsDTO : thirdMarketDTO.getMarketOddsList()) {
                            ...
                            if (null != thirdMarketCategoryField) {
                                thirdSportMarketOdds.setOddsFieldsTemplateId(thirdMarketCategoryField.getReferenceId());
                            }
...
//                        oddsWrapper.setDataSourceCode(internalDataSourceCode);
...
//                            redisService.hSet(Constant.REDIS_KEY.RONGHE_A99_THIRD_MARKET_ODDS_PRE + standardMatchInfo.getId() + ":" + e.getMarketCategoryId(), internalDataSourceCode + ":" + e.getRelationMarketId(), e, 3600*10);
                ...
                if (!thirdSportMarketMessages.isEmpty()) {
                    thirdSportMarketMessages.clear();
                }
...
//            log.info("::{}:: 百家赔批量拉取开始: {} 处理完成: {}", uuid, System.currentTimeMillis() - befTimeMain, requests.size());
            ...
            if (!thirdAndStandardMatchIdMap.isEmpty()) {
                thirdAndStandardMatchIdMap.clear();
            }
            ...
            if (!thirdMatchInfoBasedIdMap.isEmpty()) {
                thirdMatchInfoBasedIdMap.clear();
            }
...
//        thirdMarketBallHeadProcessor.thirdBasketballMarketBallHead(ballThirdMarketDTOs, thirdMatchInfoBasedIdMap, standardMatchInfoMap);
    ...
    private void cacheThirdMarketOdds(){
    ...
    public List<ThirdSportMarketMessage> n0nDataSourceOddsHandle(String linkId, StandardMatchInfo standardMatchInfo, List<ThirdSportMarketMessage> thirdSportMarketMessagesSort, Long sportId) {
        List<ThirdSportMarketMessage> resultList = new ArrayList<>(thirdSportMarketMessagesSort.size());
        try {
...
//            }
            ...
            thirdSportMarketMessagesSort.forEach(e -> {
                ...
                log.info("::{}, 玩法id:{}, 盘口值:{}, 盘口id:{}, 盘口抽水赔率设置完成:{}", linkId, e.getMarketCategoryId(), e.getAddition1(), e.getRelationMarketId(), e);
                resultList.add(e);
            });
        } catch (Exception e) {
            log.info("::{}::n0nDataSourceOddsHandle,赔率计算,异常:{}", linkId, e.getMessage(), e);
        }
        resultList.forEach(market -> {
    ...
    public void getMarketId(String linkId, StandardMatchInfo standardMatchInfo, List<ThirdSportMarketMessage> thirdSportMarketMessages) {
        try {
            ...
            thirdSportMarketMessages.stream().forEach(thirdSportMarketMessage -> {
                if (marketMap.containsKey(thirdSportMarketMessage.getThirdMarketSourceId()) && marketMap.get(thirdSportMarketMessage.getThirdMarketSourceId()) !=null) {
    ...
    /**
     * 取出三方赔率中的站点信息, 如果没有站点，就取数据源
     * @param requests
     * @return
     */
    private String getInternalDataSourceCode(List<Request<ThirdMatchMarketDTO>> requests){
        Request<ThirdMatchMarketDTO> request = requests.get(0);
        if (null != request) {
            ThirdMatchMarketDTO data = request.getData();
            List<ThirdMarketDTO> marketList = data.getMarketList();
            ThirdMarketDTO thirdMarketDTO = marketList.get(0);
            return ObjectUtil.isNotEmpty(thirdMarketDTO.getInternalDataSourceCode()) ? thirdMarketDTO.getInternalDataSourceCode() : thirdMarketDTO.getDataSourceCode();
        }
        return null;
    }
}
